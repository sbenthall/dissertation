...
and advancing a formal definition of situated information flow
...
information flow.
Drawing on the formal model of situated information flow in
...
a person and a thing that transcends
...
one implicated by that sector's activity.
...
I synthesize a formal representation
of situated information flow from Nissenbaum,
...
that carries nomic associations due to other causal
flows in its context, which can be represented
...
Dretske's \cite{dretske1981knowledge} definition
...
I define a data game as a MAID adapted into a framework
for mechanism design. This is done with an extension
to the formalism.
We introduce the mechanic of an \emph{optional edge},
...
in the graph, and one with the edge ``closed'' or absent.
...
considering a strategic equilibrium of all players in each case.

In the cases discussed in this chapter, I will consider
the strategic value of information flow except when specifically
stated otherwise.\footnote{Section \ref{sec:expert-ecs-analysis}
  discusses a case where tactical and strategic properties of
  a system are different: modeling the reaction to a pontential
security threat.}
...
Paradigmatically, more suitable employees are benefited
...
the knowledge of $V$ to make this tactical decision.
...
\end{tabular}
\end{center}
\caption[Outcomes of a principal-agent contract data game]{Outcomes for open and closed cases of the principal-agent contract data game with $V_E = \E(V)$.}
\end{table} 
...
\end{tabular}
\end{center}
\caption[Outcomes of a price differentiation data game.]{Outcomes of open and closed cases in the price differentiation data game with $V_E = \E(V)$.}
\end{table}
...
The variable $V$ encodes the value to the particular client
...
\begin{tabular}{ |c|c|c| } 
 ...
 \hline
\end{tabular}
\caption[Outcomes of an expert services data game.]{Outcomes for open and closed cases of the expert service data game.}
\end{center}
\end{table}
...
In this example, the expert chooses a strategy at
...
While we have provided narrative stories for
...
this way is to model the economic impact of the
\emph{secondary use} of data.
What economic reason makes secondary data use so popular
for businesses, even though it risks affronting social norms
and risks legal violation?
...
\begin{center}
  \begin{tikzcd}
    & & W \arrow[dl] \arrow[dr] & & \\
  & D \arrow[d] \arrow[dddl, bend right = 20] \arrow[dddl, bend right = 20] \arrow[dd, bend left = 40] & & V \arrow[dddr, bend left = 20]&  \\
...
In this model, the firm's incentives are the same
...
\end{tabular}
\end{center}
\caption[Outcomes of a cross-context flow data game.]{Outcomes of opened and closed condition for cross-context flow data game with $D_E = \E(D)$ and $V_E = \E(V)$.}
\end{table}
...
\begin{exm}
  ...
  whereas in the closed case all potential agents get hired.
\end{exm}
...
What these sectors all have in common is their firms require
significant disclosures of personal
...
This chapter has demonstrated ``data games'', a framework for modeling
...
as well as the less understood case of expert services.
...
These models make clear that the strategic choices of agents
...
This matches results from the economic models:
in this chapter,
...
because if there was perfect information in the data market,
...
Data minimization requirements reduce the amount
to which data is unintendedly exposed to other purposes.
